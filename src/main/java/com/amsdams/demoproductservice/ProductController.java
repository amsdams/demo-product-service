...
public class ProductController {
		...
		return productService.findById(id).map(product -> {
			try {
				return ResponseEntity.ok().eTag(Integer.toString(product.getVersion()))
						.location(new URI(URI_PRODUCT + product.getId())).body(product);
	...
	public ResponseEntity<Product> createProduct(@RequestBody Product product) {
		...
		try {
			...
			return ResponseEntity.created(new URI(URI_PRODUCT + newProduct.getId()))
	 ...
	 * @return A ResponseEntity that contains the updated product or one of the
	 *         following error statuses: NOT_FOUND if there is no product in the
	 *         database with the specified ID CONFLICT if the eTag does not match
	 *         the version of the product to update INTERNAL_SERVICE_ERROR if there
	 *         is a problem creating the location URI
	 */
	...
	public ResponseEntity<?> updateProduct(@RequestBody Product product, @PathVariable Integer id,
		...
		return existingProduct.map(p -> {
			// Compare the etags
			log.info("Product with ID: " + id + " has a version of " + p.getVersion() + ". Update is for If-Match: "
					+ ifMatch);
			...
			log.info("Updating product with ID: " + p.getId() + " -> name=" + p.getName() + ", quantity="
					+ p.getQuantity() + ", version=" + p.getVersion());

			try {
				// Update the product and return an ok response
				if (productService.update(p)) {
					return ResponseEntity.ok().location(new URI(URI_PRODUCT + p.getId()))
	 ...
	 * @return A ResponseEntity with one of the following status codes: 200 OK if
	 *         the delete was successful 404 Not Found if a product with the
	 *         specified ID is not found 500 Internal Service Error if an error
