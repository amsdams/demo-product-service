...
import com.amsdams.demoproductservice.ProductRepository;
import com.amsdams.demoproductservice.ProductService;
...
class ProductServiceTest {
	...
	void testUpdate() {
		Product mockProduct = new Product(1, "Product Name", 10);
		boolean mockUpdate = true;
		doReturn(mockUpdate).when(repository).update(any());

		boolean returnedProduct = service.update(mockProduct);

		Assertions.assertNotNull(returnedProduct, "The saved product should not be null");
		Assertions.assertEquals(true, returnedProduct, "The version for a new product should be true");
	}
	...
	void testDelete() {
		Product mockProduct = new Product(1, "Product Name", 10);
		boolean mockDelete = true;
		doReturn(mockDelete).when(repository).delete(any());

		boolean returnedProduct = service.delete(mockProduct.getId());

		Assertions.assertNotNull(returnedProduct, "The saved product should not be null");
		Assertions.assertEquals(true, returnedProduct,
				"The version for a new product should be 1");
	}
