...
import com.amsdams.demoproductservice.Product;
...
class ProductServiceIntegrationTest {
	...
	void testGetProductByIdFound() throws Exception {
		...
		mockMvc.perform(get("/product/{id}", 1))
				...
				.andExpect(jsonPath("$.id", is(1))).andExpect(jsonPath("$.name", is("Product 1")))
	...
	void testCreateProduct() throws Exception {
		...
		mockMvc.perform(post("/product").contentType(MediaType.APPLICATION_JSON).content(asJsonString(postProduct)))
				...
				// Validate the headers
				.andExpect(header().string(HttpHeaders.ETAG, "\"1\"")).andExpect(header().exists(HttpHeaders.LOCATION))

				// Validate the returned fields
				.andExpect(jsonPath("$.id", any(Integer.class))).andExpect(jsonPath("$.name", is("Product Name")))
				.andExpect(jsonPath("$.quantity", is(10))).andExpect(jsonPath("$.version", is(1)));
	}
	...
	void testProductPutSuccess() throws Exception {
		...
		mockMvc.perform(put("/product/{id}", 2).contentType(MediaType.APPLICATION_JSON).header(HttpHeaders.IF_MATCH, 2)
				...
				// Validate the returned fields
				.andExpect(jsonPath("$.id", is(2))).andExpect(jsonPath("$.name", is("Product 2 Updated")))
				.andExpect(jsonPath("$.quantity", is(10))).andExpect(jsonPath("$.version", is(3)));
	}
	...
	void testProductPutVersionMismatch() throws Exception {
		...
		mockMvc.perform(put("/product/{id}", 1).contentType(MediaType.APPLICATION_JSON).header(HttpHeaders.IF_MATCH, 7)
				.content(asJsonString(putProduct)))
	...
	void testProductPutNotFound() throws Exception {
		...
		mockMvc.perform(put("/product/{id}", 99).contentType(MediaType.APPLICATION_JSON).header(HttpHeaders.IF_MATCH, 1)
				.content(asJsonString(putProduct)))
	...
	void testProductDeleteNotFound() throws Exception {
		// Execute our DELETE request
		mockMvc.perform(delete("/product/{id}", 99)).andExpect(status().isNotFound());
	}
